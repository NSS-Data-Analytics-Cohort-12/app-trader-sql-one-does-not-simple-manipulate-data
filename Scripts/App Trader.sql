...
subquery1 AS (  
	SELECT
	      DISTINCT COALESCE(a.name, p.name) AS app_name 
		, GREATEST(a.price, CAST(TRIM('$' FROM p.price) AS numeric))::MONEY AS max_price 
		, CASE 
...
subquery2 AS ( 
	SELECT
		...
		, monthly_revenue - 1000::MONEY AS monthly_income  
